...
async function ghLerPrecos() {
  // Lê o JSON directamente via GitHub Pages (igual ao site público)
  // Evita problemas de base64 da GitHub API
  const urlPublico = `https://${REPO_OWNER}.github.io/${REPO_NAME}/data/precos.json?_=${Date.now()}`;
  const respPublico = await fetch(urlPublico);
  if (!respPublico.ok) throw new Error(`Erro a ler precos.json: HTTP ${respPublico.status}`);
  const conteudo = await respPublico.json();

  // Vai buscar o SHA via API (necessário para poder escrever depois)
  const respApi = await ghFetch(`/repos/${REPO_OWNER}/${REPO_NAME}/contents/${FILE_PATH}?ref=${BRANCH}`);
  if (!respApi.ok) {
    const err = await respApi.json().catch(() => ({}));
    throw new Error(err.message || `API HTTP ${respApi.status}`);
  }
  const meta = await respApi.json();
...
/* ---------------------------------------------------------------------- */
/* Atualizar preços — dispara o workflow do GitHub Actions (scraping)     */
/* ---------------------------------------------------------------------- */

const WORKFLOW_FILE = "scraping.yml";

/** Dispara o workflow de scraping via GitHub Actions */
async function ghDispararScraping() {
...
/** Vai buscar a execução mais recente do workflow (para saber o estado) */
async function ghUltimaExecucao() {
  const resp = await ghFetch(
...
els.btnAtualizar.addEventListener("click", async () => {
  if (estado.alterado) {
    if (!confirm("Tens alterações por publicar. Se atualizares agora, essas alterações locais "
      + "podem ser substituídas quando o scraping publicar novos preços. Continuar mesmo assim?")) {
      return;
    }
  }
  if (!confirm("Isto vai correr o scraping completo no GitHub Actions (demora vários minutos). "
    + "Continuar?")) {
    return;
  }

  const btn = els.btnAtualizar;
  btn.disabled = true;
  btn.textContent = "A iniciar…";
  setStatus("espera", "A disparar o scraping no GitHub Actions…");

  try {
    // Guardar timestamp de antes de disparar, para identificar a nova execução
    const antesDoDisparo = new Date();
    await ghDispararScraping();

    toast("Scraping iniciado! Isto corre no GitHub Actions e pode demorar vários minutos.", "sucesso");
    btn.textContent = "A correr…";

    // Poll: espera a execução aparecer e acompanha o estado
    let execucao = null;
    let tentativas = 0;
    while (tentativas < 20) {
      await new Promise(r => setTimeout(r, 3000));
      execucao = await ghUltimaExecucao();
      tentativas++;
      if (execucao && new Date(execucao.created_at) >= antesDoDisparo) break;
      execucao = null;
    }

    if (!execucao) {
      toast("Não consegui confirmar o início — verifica o separador Actions no GitHub.", "erro");
      setStatus("erro", "Confirma no GitHub → Actions");
      btn.disabled = false;
      btn.textContent = "\u21BB Atualizar preços";
      return;
    }

    // Continuar a fazer poll até terminar (status: queued/in_progress → completed)
    while (execucao.status !== "completed") {
      setStatus("espera", `Scraping a correr… (${execucao.status})`);
      await new Promise(r => setTimeout(r, 8000));
      execucao = await ghUltimaExecucao();
    }

    if (execucao.conclusion === "success") {
      toast("Scraping concluído! A recarregar preços…", "sucesso");
      setStatus("ok", "Scraping concluído");
      await carregarPrecos();
    } else {
      toast(`Scraping terminou com estado: ${execucao.conclusion}. Verifica o GitHub Actions.`, "erro");
      setStatus("erro", `Scraping: ${execucao.conclusion}`);
    }
  } catch (e) {
    toast("Erro ao disparar scraping: " + e.message, "erro");
    setStatus("erro", "Erro: " + e.message);
  } finally {
    btn.disabled = false;
    btn.textContent = "\u21BB Atualizar preços";
  }
